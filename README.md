...
- `/web on` — включить веб-поиск (DuckDuckGo), `/web off` — выключить

## Доступ в интернет: `/web`

По умолчанию бот отвечает из знаний модели. Команда `/web on` включает режим веб-поиска: если модель решит, что нужен свежий факт, она вызовет инструмент `web_search` — поиск идёт через **DuckDuckGo** (`ddgs`), **без API-ключа**.

```text
/web on        — включить
/web off       — выключить
/web           — показать текущий статус
```

Параметры поиска:
- `WEB_MODEL` — модель для web-режима (по умолчанию `glm-4.6`, подтверждённый function calling). Когда web-режим включён, для всего обмена (decision + финальный ответ) используется `WEB_MODEL`, а не выбранная через `/model`.
- Результаты: 5 шт., обрезаются до ~300 символов, чтобы не раздувать контекст.
- Если DuckDuckGo недоступен или превышен таймаут 5 сек — модель ответит «поиск недоступен» и не упадёт.
